...
// The "General" tab on Organization Settings — owns its own query/mutation,
// same "each tab fetches its own data" convention as IndividualSchedulesTab/
// BulkSchedulesTab on Manage Schedules. Mounted only while this tab is
// active, so mounting itself is the fetch trigger (no `enabled` flag needed).
export function GeneralSettingsTab() {
  ...
  if (isError) {
    return <Text style={styles.errorText}>{getApiErrorMessage(error, 'Failed to load organization info.')}</Text>;
  }
